...
<p align="center">
  <img src="./assets/vocalis-logo.png" width="450px">
</p>
...
<div style="display: flex; gap: 10px;">
  <img src="./assets/DA AGGIUNGERE" alt="Home screen 1" width="45%"/>
</div>

### Videochiamata

<div style="display: flex; gap: 10px;">
  <img src="./assets/DA AGGIUNGERE" alt="Video call screen" width="45%"/>
</div>

### Traduzione in tempo reale

<div style="display: flex; gap: 10px;">
  <img src="./assets/DA AGGIUNGERE" alt="Translation screen" width="45%"/>
</div>
...
### Video Tutorial
<div style="display: flex; gap: 10px;">
  <video src="./assets/DA AGGIUNGERE" width="100%" controls></video>
</div>
...
`VideoRoom` riceve `username` e `roomID` da `HomePage`, permettendono di istanziare dei riferimenti con questi parametri tramite l'hook `useRef`.
...
Il vero nucleo tecnico di `VideoRoom.js` risiede nel modo in cui gestisce il protocollo **JSEP**, un automa a stati finiti essenziale per stabilire connessioni WebRTC (scambi Offer/Answer) coordinandosi con il Gateway Janus tramite WebSockets. Attraverso i WebSocket scambiamo i pacchetti `SDP` (Session Description Protocol) per negoziare i codec audio/video e raccogliamo i candidati `ICE` per oltrepassare eventuali Firewall/NAT.

***Flusso "Publisher"***
Quando un utente entra, si attacca inizialmente come **Publisher** (chi trasmette stream) inviando una richiesta `join` e avvia la fase di negoziazione creando un'offerta SDP (`createOffer`), in cui specifica i flussi multimediali (audio/video) e l'apertura esplicita del DataChannel per i testi:
```javascript
...
```
Janus riceve questa offerta, alloca le porte ice, e risponde a sua volta con una **Answer** contenente un pacchetto JSEP remoto. La risposta viene intercetta tramite un handler `handleRemoteJsep({ jsep: jsep })`. Come piano di riserva, se la fotocamera non fosse autorizzata dal browser, l'automa è programmato con un fallback "Solo Dati", rigenerando un'offerta in cui è specificato unicamente il DataChannel.

***Flusso "Subscriber"***
Ogni volta che Janus notifica la presenza di un nuovo membro nella stanza (evento `msg["publishers"]`), `VideoRoom` invoca la funzione `newRemoteFeed()`. Tramite questa funzione riusciamo a gestire l'offerta SDP che Janus ci invia per farci iscrivere al feed remoto, alla stregua del `setRemoteDescription` del protocollo JSEP. Il client elabora questo JSEP in entrata rispondendo con una **Answer** SDP tramite `createAnswer`, accettando l'A/V passivamente e richiedendo l'aggancio dati:
```javascript
...
Inoltre il componente espone un bottone "Esci" (`onLeave`) che chiama `janusRef.current.destroy()`. Questo azzera forzatamente l'automa di stato, dealloca le PeerConnection distruggendo gli hook JSEP e pulendo la memoria per evitare colli di bottiglia nel browser, prima di riportare l'utente alla `HomePage`.
